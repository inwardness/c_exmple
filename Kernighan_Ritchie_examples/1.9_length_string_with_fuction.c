...
int main(){
	   ...
	   while((len=fgetline(line,lim))>0)				//we get our string until it not equal zero 
		if(len>max){						// len compare value of count od line parametre in function fgetline with value of maximum
									// for start it's zero
		...
		int fgetline(unsigned char s[],int lim){		//function to get symbol from standart input by condition and move to massive
		 int c,i;
